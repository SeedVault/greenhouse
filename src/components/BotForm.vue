...
export default {
  ...
  setup(props, context) {
    ...
    async function configureComponent(event) {
      data.configForUser = await getPropertiesForValueType(event.id, 'developer');
      data.cachedComponents.set(data.configForUser._id, data.configForUser);
      data.configValues = {
        component: data.configForUser._id,
        subscriptionType: '',
        values: new Map(),
      };
      switch (data.configForUser.pricingModel) {
        case 'free':
